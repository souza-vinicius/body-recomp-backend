...
export function EditProgressModal({ entry, onSave, onClose }: EditProgressModalProps) {
  const t = useTranslations('Progress.EditModal');
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-[100] p-0 sm:p-4 animate-fade-in" onClick={onClose}>
      <div
        className="bg-surface w-full sm:max-w-md sm:rounded-2xl rounded-t-2xl shadow-elevated animate-slide-up max-h-[85dvh] flex flex-col overflow-hidden"
        onClick={e => e.stopPropagation()}
      >
        <div className="px-6 pt-6 pb-3 border-b border-surface-100 flex-shrink-0 bg-surface">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-black text-surface-900">{t('title')}</h2>
        ...
        <div className="p-6 space-y-5 overflow-y-auto flex-1 min-h-0 custom-scrollbar">
          {/* Stats summary */}
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-surface-50 rounded-xl p-3 text-center">
              <Calendar size={14} className="text-surface-400 mx-auto mb-1" />
              <span className="text-xs text-surface-400 block">{t('week')}</span>
            ...
            <div className="bg-surface-50 rounded-xl p-3 text-center">
              ...
              <span className="text-xs text-surface-400 block">{t('weight')}</span>
            ...
            <div className="bg-primary-50 rounded-xl p-3 text-center">
              ...
              <span className="text-xs text-primary-500 block">{t('body_fat')}</span>
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="edit-date" className="text-sm font-semibold text-surface-700">{t('entry_date')}</label>
            <input
              id="edit-date"
              type="date"
              value={entryDate}
              onChange={e => setEntryDate(e.target.value)}
              className="px-3.5 py-2.5 text-sm bg-surface border-2 border-surface-200 rounded-xl shadow-sm transition-all duration-200 focus:outline-none focus:ring-0 focus:border-primary-500 hover:border-surface-300"
            />
          </div>
          ...
          <div className="space-y-3 p-4 bg-surface-50 rounded-xl border border-surface-100">
            <h4 className="flex items-center gap-2 text-sm font-bold text-surface-900">
              ...
              {t('circumferences_title')}
            </h4>
            <p className="text-xs text-surface-400">{t('circumferences_hint')}</p>
          ...
          {/* Notes */}
          <div className="flex flex-col gap-1.5">
            <label htmlFor="edit-notes" className="text-sm font-semibold text-surface-700">{t('notes_label')}</label>
            <textarea
              id="edit-notes"
              value={notes}
              ...
              rows={3}
              maxLength={1000}
              className="px-3.5 py-2.5 text-sm bg-surface border-2 border-surface-200 rounded-xl shadow-sm transition-all duration-200 focus:outline-none focus:ring-0 focus:border-primary-500 hover:border-surface-300 placeholder:text-surface-300 resize-none"
        ...
        <div className="p-6 border-t border-surface-100 flex-shrink-0 bg-surface">
